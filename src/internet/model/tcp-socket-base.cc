...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if(!outItem) NS_LOG_DEBUG("outItem is null");
  else NS_LOG_DEBUG("outItem is not null");
...
TcpSocketBase::SendPendingData (bool withAck)
{
  ...
  while (availableWindow > 0)
    {
      ...
      if (!m_txBuffer->NextSeg (&next, &nextHigh, enableRule3))
      ...
      else
        {
          ...
          NS_LOG_DEBUG("availableWindow:"<<availableWindow);
          ...
          NS_LOG_DEBUG("maxSizeToSend:"<<maxSizeToSend<<",nextHigh="<<nextHigh<<",next="<<next);
  ...
  if (nPacketsSent > 0)
    ...
    NS_LOG_DEBUG("end SendPendingData");
  return nPacketsSent;
}
