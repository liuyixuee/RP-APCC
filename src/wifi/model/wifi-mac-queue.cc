...
WifiMacQueue::DoDequeue (ConstIterator pos)
{
  ...
  if(m_ac==AC_BE) NS_LOG_DEBUG ("current queue dequeue: AC_BE"  << " at "<<m_lastSendTime.GetSeconds());
...
// Zhuge
Time WifiMacQueue::GetLastSendTime()
{
  return m_lastSendTime;
}

void WifiMacQueue::SetLastSendTime(Time lastSendTime)
{
  m_lastSendTime=lastSendTime;
}
