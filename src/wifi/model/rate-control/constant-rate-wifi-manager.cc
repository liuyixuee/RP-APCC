...
ConstantRateWifiManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ConstantRateWifiManager")
    ...
    .AddAttribute ("ControlMode", "The transmission mode to use for every RTS packet transmission.",
                   ...
                   MakeWifiModeChecker ())
    .AddAttribute ("RateChange",
                   "True, Transmission Rate will be changed",
                   BooleanValue (false),
                   MakeBooleanAccessor (&ConstantRateWifiManager::m_rateChange),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
{
  NS_LOG_FUNCTION (this << station);
  //NS_LOG_UNCOND ("Failed:::" << " "<< Simulator::Now().GetNanoSeconds() <<" "<<this->m_dataMode);
  uint64_t now = Simulator::Now().GetNanoSeconds();
  uint64_t time_down=30000000000;
  uint64_t time_up=35000000000;
  bool changerate_down=m_rateChange;
  bool changerate_up=m_rateChange;
  if(changerate_down&&now>time_down&&now<time_up&& this->m_dataMode!=WifiMode("HeMcs2"))
  {
    NS_LOG_UNCOND("CHANGE RATE TO HEMCS2");
    this->m_dataMode=WifiMode("HeMcs2");
  } 
  
  if(changerate_up&&now>time_up&& this->m_dataMode!=WifiMode("HeMcs11"))
  {
    NS_LOG_UNCOND("CHANGE RATE TO HEMCS11");
    this->m_dataMode=WifiMode("HeMcs11");
  } 
}
                                         ...
                                         double dataSnr, uint16_t dataChannelWidth, uint8_t dataNss)
{
  ...
  bool changerate_down=m_rateChange;
  bool changerate_up=m_rateChange;
  //NS_LOG_UNCOND ("Successed:::" << " "<< Simulator::Now().GetNanoSeconds() <<" "<<this->m_dataMode);
  if(changerate_down&&now>time_down&&now<time_up&& this->m_dataMode!=WifiMode("HeMcs2"))
  {
    NS_LOG_UNCOND("CHANGE RATE TO HEMCS2");
    this->m_dataMode=WifiMode("HeMcs2");
