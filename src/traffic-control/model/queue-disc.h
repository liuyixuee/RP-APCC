...
private:
  ...
  //Zhuge
  public:
  Time m_lastDequeueTime; //用于计算当前队头包的等待时间--qShort
  std::deque<std::pair<double,double>> m_dequeueInterval; //一段时间窗口内的出队列时间，用于平均--tx
  double m_avgDequeueInterval;
  uint32_t m_txSlidingWindow; //计算呢两个tx的窗口大小，deque设置大小
  double m_txSlidingWindow_time;
  double m_sumDeququeInterval;
};
