     ...
     m_prohibitChangeMode (false)
{
  ...
  m_childQueueDiscMarkFunctor = [this] (Ptr<const QueueDiscItem> item, const char* r)
    ...
    //Zhuge initialization
    m_lastDequeueTime = Simulator::Now(); 
    m_avgDequeueInterval=0;
    // todo: 窗口设置
    m_txSlidingWindow=100; //计算呢两个tx的窗口大小，deque设置大小
    m_txSlidingWindow_time = 0.04;
    m_sumDeququeInterval=0;
}
...
QueueDisc::Enqueue (Ptr<QueueDiscItem> item)
{
  ...
  NS_LOG_DEBUG("queue-disc.cc: ready to quit enqueue");
  return retval;
}
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG("queue-disc.cc: in Dequeue()");
  ...
  if (item)
    {
      NS_LOG_DEBUG("queue-disc.cc: requeued packet");
      m_requeued = 0;
      if (m_peeked)
        {
  ...
  else
    {
      ...
      //     NS_LOG_DEBUG("queue-disc.cc: dequeue an ACK packet");
      //     double sendTime = item->m_sendTime;
      //     while(Simulator::Now().GetSeconds() < sendTime)
      //     {
      //     }
          
      //   }
      //   else{
      //     // Zhuge: update last dequeue time for data packets(downlink)
          //NS_LOG_UNCOND("queue-disc.cc: dequeue a data packet, record related information");
          double dequeueInterval = Simulator::Now().GetSeconds() - m_lastDequeueTime.GetSeconds();
          while(!m_dequeueInterval.empty()&&m_dequeueInterval.front().first< Simulator::Now().GetSeconds()- m_txSlidingWindow_time)
          {
            m_sumDeququeInterval -= m_dequeueInterval.front().second;
            m_dequeueInterval.pop_front();
          }
          
          m_dequeueInterval.push_back(std::pair<double,double>(Simulator::Now().GetSeconds(),dequeueInterval));
          m_sumDeququeInterval += dequeueInterval;
          m_avgDequeueInterval = m_sumDeququeInterval / m_dequeueInterval.size();
  ...
  NS_LOG_DEBUG("queue-disc.cc: dequeue finished");
